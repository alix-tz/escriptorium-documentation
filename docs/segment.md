...
This section introduces the **segmentation panel** and some related features. The segmentation panel is [one of the panels available](terminology.md#edition-panels) in the "Edit" tab, once you are in a document (`{base_url}/document/{document-id}/images/`).
...
- Click on the image where you want the line to start and then click on the image where you want the line to stop. It will create a simple line passing through two points.
...
To create your own types, write the desired name for the new region in the dedicated field and then click on the **`+`** button. Again, remember to click on the "Update" button to save the changes.
